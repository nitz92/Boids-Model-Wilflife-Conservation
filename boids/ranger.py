...
class Ranger:
    ...
    def determine_nearby_rangers(self, all_rangers):
    ...
    def attraction(self, attractors):
        # generate a vector that moves the ranger towards the poachers
        a = [0.0, 0.0]
        if not attractors:
            return a
    ...
    def update(self, dt, all_rangers, attractors):
        nearby_rangers = list(self.determine_nearby_rangers(all_rangers))
        ...
        ranger_avoidance_vector = self.avoid_collisions(all_rangers, _RANGER_COLLISION_DISTANCE)
